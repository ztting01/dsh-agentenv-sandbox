...
This is an MVP for DeepSeek Harness `0.1.0-rc.7`, the matching published E2B provider family `0.1.0-rc.7`, and E2B SDK `2.29.1`. It has live compatibility coverage for AgentENV `0.1.0` and `0.1.3`; the `0.1.3` test covers authenticated sandbox creation, workspace upload, filesystem operations, non-interactive commands, persistent PTYs, teardown, and a real Harness Web-profile startup. Its bundle composition has also been checked with a Harness `0.1.0-rc.8` source checkout. Run Harness inside WSL 2 or Linux. Native Windows execution is rejected because one Harness session cwd must be a valid path in both the host process and remote Linux world.
...
- AgentENV `0.1.3` installed, authenticated, and running (`0.1.0` remains supported). This plugin does not install or start AgentENV.
...
- [AgentENV v0.1.3 source and deployment documentation](https://github.com/kvcache-ai/AgentENV/tree/v0.1.3)
...
This command uses the AgentENV `0.1.3` CLI syntax tested by this plugin. AgentENV `0.1.0` used `--tag` instead of `--name`; always use the CLI syntax matching the deployed server version.
...
For normal use, download the prebuilt release tarball and add it to a Harness profile. Release users do **not** need to clone this repository, run `npm install`, or run `npm run check`; the tarball already contains the compiled `lib` files:
...
#### Plugin source development only

The following commands are only for contributors who cloned or modified the plugin source. Run them from the plugin repository root — the directory containing this plugin's `package.json` — not from an ordinary project workspace:
...
`npm install` reads this repository's `package.json` and lockfile, downloads the plugin's Node.js runtime and development dependencies from the npm registry, and places them under this repository's `node_modules`. Those dependencies include the E2B SDK, the official DSH E2B providers, TypeScript, Vitest, and their transitive packages. It does **not** install AgentENV, build the AgentENV template, install the DeepSeek Harness application, or install this plugin into a Harness profile.

`npm run check` runs TypeScript type checking, unit tests, and the production build in sequence. It creates the compiled `lib` output required when installing directly from the source directory. It does not start AgentENV or perform the live AgentENV smoke test; use `npm run smoke:agentenv` separately for that after completing the AgentENV setup.
...
The script uses `E2B_API_URL` plus `E2B_API_KEY`/`AENV_API_KEY` when both are set. Otherwise it reads the existing `aenv auth` credential file, or the file named by `AENV_CREDENTIALS_PATH`. It never prints the API key. The test creates a short-lived sandbox, verifies bidirectional visibility between `ctx.fs` and `ctx.subprocess`, verifies PTY output, proves there is no host write-back, and deletes the sandbox during teardown.
...
- AgentENV 0.1.0 and 0.1.3 ship envd 0.5.15, while the official FS provider requests file metadata introduced in envd 0.6.2. The plugin retries only that explicitly unsupported metadata write without metadata. Ordinary metadata fingerprints still detect external changes, but identical-size writes inside an extremely coarse timestamp window have weaker stale-version detection until AgentENV upgrades envd.
